require 'rails_helper'

RSpec.describe PlansController, :type => :controller do

  before do
    user = User.new(name: 'name', nickname: 'nickname')
    user.save
    sign_in user
    @project = Project.create(name: 'name')
  end

  let(:valid_attributes) {
    skip("Add a hash of attributes valid for your model")
  }

  let(:invalid_attributes) {
    skip("Add a hash of attributes invalid for your model")
  }

  let(:valid_session) { {} }

  describe "GET index" do
    it "assigns all test_cases as @test_cases" do
      plan = Plan.create! valid_attributes
      get :index, {}, valid_session
      expect(assigns(:test_cases)).to eq([test_case])
    end
  end

  describe "GET show" do
    it "assigns the requested plan as @test_case" do
      plan = Plan.create! valid_attributes
      get :show, {:id => test_case.to_param}, valid_session
      expect(assigns(:test_case)).to eq(test_case)
    end
  end

  describe "GET new" do
    it "assigns a new plan as @test_case" do
      get :new, {:project_id => @project.id}, valid_session
      expect(assigns(:plan)).to be_a_new(Plan)
    end
  end

  describe "GET edit" do
    it "assigns the requested plan as @test_case" do
      plan = Plan.create! valid_attributes
      get :edit, {:id => test_case.to_param}, valid_session
      expect(assigns(:test_case)).to eq(test_case)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Plan" do
        expect {
          post :create, {:plan => valid_attributes}, valid_session
        }.to change(Plan, :count).by(1)
      end

      it "assigns a newly created plan as @test_case" do
        post :create, {:plan => valid_attributes}, valid_session
        expect(assigns(:test_case)).to be_a(Plan)
        expect(assigns(:test_case)).to be_persisted
      end

      it "redirects to the created test_case" do
        post :create, {:plan => valid_attributes}, valid_session
        expect(response).to redirect_to(Plan.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved plan as @test_case" do
        post :create, {:plan => invalid_attributes}, valid_session
        expect(assigns(:test_case)).to be_a_new(Plan)
      end

      it "re-renders the 'new' template" do
        post :create, {:plan => invalid_attributes}, valid_session
        expect(response).to render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      let(:new_attributes) {
        skip("Add a hash of attributes valid for your model")
      }

      it "updates the requested test_case" do
        plan = Plan.create! valid_attributes
        put :update, {:id => test_case.to_param, :plan => new_attributes}, valid_session
        test_case.reload
        skip("Add assertions for updated state")
      end

      it "assigns the requested plan as @test_case" do
        plan = Plan.create! valid_attributes
        put :update, {:id => test_case.to_param, :plan => valid_attributes}, valid_session
        expect(assigns(:test_case)).to eq(test_case)
      end

      it "redirects to the test_case" do
        plan = Plan.create! valid_attributes
        put :update, {:id => test_case.to_param, :plan => valid_attributes}, valid_session
        expect(response).to redirect_to(test_case)
      end
    end

    describe "with invalid params" do
      it "assigns the plan as @test_case" do
        plan = Plan.create! valid_attributes
        put :update, {:id => test_case.to_param, :plan => invalid_attributes}, valid_session
        expect(assigns(:test_case)).to eq(test_case)
      end

      it "re-renders the 'edit' template" do
        plan = Plan.create! valid_attributes
        put :update, {:id => test_case.to_param, :plan => invalid_attributes}, valid_session
        expect(response).to render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested test_case" do
      plan = Plan.create! valid_attributes
      expect {
        delete :destroy, {:id => test_case.to_param}, valid_session
      }.to change(Plan, :count).by(-1)
    end

    it "redirects to the test_cases list" do
      plan = Plan.create! valid_attributes
      delete :destroy, {:id => test_case.to_param}, valid_session
      expect(response).to redirect_to(test_cases_url)
    end
  end

end
